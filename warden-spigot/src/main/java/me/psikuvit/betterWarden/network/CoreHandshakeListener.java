...
/**
 * Receives the {@code warden:core} handshake a proxy sends when a player first connects to this
 * backend, and caches it to disk - identical to warden-paper's own CoreHandshakeListener (plain
 * Bukkit API throughout, nothing Paper-specific, so it's duplicated rather than shared - warden-core
 * stays Bukkit-free by design). Only takes effect on the *next* boot (warden.mode: client).
 */
public class CoreHandshakeListener implements PluginMessageListener {
    ...
    /** Read back on a CLIENT boot - see WardenSpigotPlugin.java. */
